...
model {
  ...
  // log TPMs, detection logits, and initial state log probabilities
  ...
  if (ind) {
    ...
    if (collapse) {
      ...
      tuple(vector[I], vector[2], matrix[J, I], vector[J], 
        ...
        js_ms2(y, f_l, log_H_i, logit_p_i, log_beta, log_eta_i, psi);
      ...
      tuple(vector[I], matrix[2, I_aug], matrix[J, I], matrix[J, I_aug], 
        ...
        js_ms(y, f_l, log_H_i, logit_p_i, log_beta, log_eta_i, psi);
      target += lp.1;
      for (i in 1:I_aug) {
    ...
    tuple(vector[I], vector[2], matrix[J, I], vector[J], array[I] matrix[S, J],
          matrix[S, J]) lp =
      js_ms(y, f_l, log_H_j, logit_p_j, log_beta, log_eta_j, psi);
    target += sum(lp.1) + I_aug * log_sum_exp(lp.2);
  }
}
  ...
  int N_super;
  {
    ...
    if (ind) {
      if (collapse) {
        ...
        tuple(vector[I], vector[2], matrix[J, I], vector[J], 
              array[I] matrix[S, J], matrix[S, J]) lp = 
          js_ms2(y, f_l, log_H_i[:, :, :S, :S], logit_p_i, log_beta, log_eta_i, 
                 psi);
        log_lik = lp.1;
        latent = js_ms2_rng(lp, y, f_l, log_H_i, logit_p_i, I_aug);
      } else {
        ...
        tuple(vector[I], matrix[2, I_aug], matrix[J, I], matrix[J, I_aug], 
              array[I] matrix[S, J], array[I_aug] matrix[S, J]) lp = 
          js_ms(y, f_l, log_H_i[:, :, :S, :S], logit_p_i, log_beta, log_eta_i, 
                psi);
        log_lik = lp.1;
        latent = js_ms_rng(lp, y, f_l, log_H_i, logit_p_i, I_aug);
      }
    } else {
      tuple(vector[I], vector[2], matrix[J, I], vector[J], 
            array[I] matrix[S, J], matrix[S, J]) lp =
        js_ms(y, f_l, log_H_j[:, :S, :S], logit_p_j, log_beta, log_eta_j, psi);
      log_lik = lp.1;
      latent = js_ms_rng(lp, y, f_l, log_H_j, logit_p_j, I_aug);
    }
    N = latent.1;
    B = latent.2;
    D = latent.3;
