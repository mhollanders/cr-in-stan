...
model {
  ...
  if (ind) {
    ...
    if (collapse) {
      ...
      array[Ip1, J, K_max] matrix[S, S] log_E_i = rep_array(log_E_j, Ip1);
      ...
      tuple(vector[I], vector[2], matrix[J, I], vector[J], 
        ...
        js_me_rd2(y, f_l, K, log_H_i, logit_p_i, log_E_i, log_beta, log_eta_i, 
                  psi);
      ...
      array[I_all, J, K_max] matrix[S, S] log_E_i = rep_array(log_E_j, I_all);
      ...
      tuple(vector[I], matrix[2, I_aug], matrix[J, I], matrix[J, I_aug], 
        ...
        js_me_rd(y, f_l, K, log_H_i, logit_p_i, log_E_i, log_beta, log_eta_i, 
                 psi);
      target += lp.1;
      for (i in 1:I_aug) {
    ...
    tuple(vector[I], vector[2], matrix[J, I], vector[J], array[I] matrix[S, J],
          matrix[S, J]) lp =
      js_me_rd(y, f_l, K, log_H_j, logit_p_j, log_E_j, log_beta, log_eta_j, psi);
    target += sum(lp.1) + I_aug * log_sum_exp(lp.2);
  }
}
      ...
      if (collapse) {
        ...
        tuple(vector[I], vector[2], matrix[J, I], vector[J], 
          ...
          js_me_rd2(y, f_l, K, log_H_i, logit_p_i, log_E_i, log_beta, log_eta_i, 
                    psi);
        log_lik = lp.1;
        latent = js_me_rd2_rng(lp, y, f_l, K, log_H_i, logit_p_i, log_E_i, 
                               I_aug);
      } else {
        array[I_all, Jm1] matrix[Sp1, Sp1] log_H_i = rep_array(log_H_j, I_all);
        array[I_all, J] matrix[S, K_max] logit_p_i = rep_array(logit_p_j, I_all);
        ...
        tuple(vector[I], matrix[2, I_aug], matrix[J, I], matrix[J, I_aug], 
          ...
          js_me_rd(y, f_l, K, log_H_i, logit_p_i, log_E_i, log_beta, log_eta_i, 
                   psi);
        log_lik = lp.1;
        latent = js_me_rd_rng(lp, y, f_l, K, log_H_i, logit_p_i, log_E_i, 
                              I_aug);
      }
    } else {
      tuple(vector[I], vector[2], matrix[J, I], vector[J], 
            array[I] matrix[S, J], matrix[S, J]) lp =
        js_me_rd(y, f_l, K, log_H_j, logit_p_j, log_E_j, log_beta, log_eta_j, 
                 psi);
      log_lik = lp.1;
      latent = js_me_rd_rng(lp, y, f_l, K, log_H_j, logit_p_j, log_E_j, I_aug);
    }
    N = latent.1;
    B = latent.2;
    D = latent.3;
